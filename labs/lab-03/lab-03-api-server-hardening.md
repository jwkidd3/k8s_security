...
# Create the cluster (must run from labs/setup so kind can find the mounted files)
cd labs/setup
kind create cluster --name api-hardening --config kind-config-api-hardening.yaml
cd ../..
...
kubectl exec -n kube-system etcd-api-hardening-control-plane -- etcdctl \
