...
kubectl exec -n kube-system etcd-security-lab-control-plane -- etcdctl \
...
```bash
kubectl apply -f labs/lab-06/secret-volume-pod.yaml
...
```bash
kubectl apply -f labs/lab-06/secret-env-pod.yaml
...
kubectl apply -f labs/lab-06/read-db-credentials-only-role.yaml

# Bind the role to the service account
kubectl apply -f labs/lab-06/app-reader-db-creds-rolebinding.yaml
...
kubectl apply -f labs/lab-06/default-sa-pod.yaml
...
kubectl apply -f labs/lab-06/no-sa-token-pod.yaml
...
kubectl apply -f labs/lab-06/no-token-sa-serviceaccount.yaml
...
kubeseal --controller-name=sealed-secrets --controller-namespace=kube-system \
  --format yaml < /tmp/my-secret.yaml > /tmp/sealed-secret.yaml
