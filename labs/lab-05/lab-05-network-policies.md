...
kubectl apply -f labs/lab-05/database-nginx-config-resources.yaml

# Deploy database tier
kubectl apply -f labs/lab-05/database-resources.yaml

# Deploy backend tier
kubectl apply -f labs/lab-05/backend-resources.yaml

# Deploy frontend tier (uses default nginx port 80)
kubectl apply -f labs/lab-05/frontend-resources.yaml
...
```bash
kubectl apply -f labs/lab-05/default-deny-ingress-networkpolicy.yaml
...
kubectl apply -f labs/lab-05/default-deny-egress-networkpolicy.yaml

echo "Default deny egress applied."

# Allow DNS resolution for all pods
kubectl apply -f labs/lab-05/allow-dns-networkpolicy.yaml
...
kubectl apply -f labs/lab-05/frontend-egress-to-backend-resources.yaml

# Allow backend -> database (egress from backend, ingress to database)
kubectl apply -f labs/lab-05/backend-egress-to-database-resources.yaml
...
kubectl apply -f labs/lab-05/prometheus-sim-pod.yaml
...
kubectl apply -f labs/lab-05/allow-monitoring-ingress-networkpolicy.yaml

# Also allow egress from the monitoring namespace to three-tier
kubectl apply -f labs/lab-05/allow-egress-to-three-tier-networkpolicy.yaml
